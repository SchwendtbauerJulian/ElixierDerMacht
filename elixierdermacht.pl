...
ort_emoji(wald, '🌲').
ort_emoji(schmiede, '🔨').
ort_emoji(bibliothek, '📚').
ort_emoji(marktplatz, '🛒').
ort_emoji(dunkle_festung, '🏰🌒').
ort_emoji(friedhof, '🪦').
...
% Namen ausgeben
schreibe_name(waldkraeuter_wilma) :- write('Waldkraeuter Wilma 🌿').
...
zeige_leben :-
    leben_spieler(Leben),
    format('Du hast ~w Leben.❤️~n', [Leben]).
...
reaktion(doppelschlag) :-
    ...
    write('Du fuehrst einen maechtigen ⚔ aus! 10 Schaden an tyrael!'), nl,
    check_kampfende,
    (
        LTneu > 0
        ->  gegner_angriff
...
reaktion(doppelschlag) :-
    ...
    format('Du fuehrst einen ⚔ aus. 15 Schaden an ~w', [Gegner]), nl,
...
reaktion(wassermagie) :-
    ...
    retract(leben_tyrael(LT)),
    assertz(leben_tyrael(LTneu)),
    write('Du setzt 🌊 ein! 20 Schaden an Tyrael!'), nl,
    check_kampfende,
    (
        LTneu > 0
        ->  gegner_angriff
...
reaktion(wassermagie) :-
    im_kampf(Gegner),
    format('Du setzt 🌊 ein. ~w ist immun gegen das Wasser', [Gegner]), nl,
    gegner_angriff(Gegner).

reaktion(feuermagie) :-
    im_kampf(tyrael),
    write('Du setzt 🔥 ein, aber Tyrael ist immun!'), nl,
    gegner_angriff.    
...
reaktion(feuermagie) :-
    ...
    retract(leben_monster(Gegner, LM)),
    assertz(leben_monster(Gegner, LMneu)),
    format('Du setzt 🔥 ein. 25 Schaden an ~w', [Gegner]), nl,
    check_kampfende(Gegner),
    (
        LMneu > 0
        ->  gegner_angriff(Gegner)
...
trinke(lebenstrank) :-
    ...
    write('Du trinkst einen Lebenstrank und bekommst +10 Leben!❤️‍🩹'), nl,
    (
        im_kampf(Gegner)
        ->  check_kampfende(Gegner),
            gegner_angriff(Gegner)
